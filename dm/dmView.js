...
document.addEventListener("DOMContentLoaded", () => {
  const toggleBtn = document.getElementById("toggleDM");
  const sheetView = document.getElementById("sheetView");
  const dmView = document.getElementById("dmView");

  toggleBtn.addEventListener("click", () => {
    console.log("DM toggle clicked");

    // FORCE visibility
    dmView.hidden = false;
    dmView.style.display = "block";
    dmView.style.border = "4px solid red";

    sheetView.hidden = true;

    console.log("dmView hidden:", dmView.hidden);
    console.log("dmView display:", getComputedStyle(dmView).display);
  });
});
await loadMonsters();
...
monsters.forEach((m, i) => {
  ...
  opt.value = i;                 // index-based reference for now
  opt.textContent = m.title;     // <-- THIS is the key
...
document.getElementById("addMonsterBtn")?.addEventListener("click", () => {
  ...
  const monster = monsters[Number(select.value)];
  if (!monster) return;

  const baseName = monster.title;

  const count =
    dmState.encounter.combatants.filter(c =>
  ...
  dmState.encounter.combatants.push({
    ...
    conditions: [],
    monsterRef: monster // ← IMPORTANT for card rendering later
  });
...
if (encounterNameInput) {
  encounterNameInput.value = dmState.encounter.name ?? "";

  encounterNameInput.addEventListener("input", e => {
...
function rollHpFromMonster(monster) {
  ...
  // Example: "property | Hit points | 142 (15d10 + 60)"
  const diceMatch = hpLine.match(/(\d+)d(\d+)\s*\+\s*(\d+)/);

  if (!diceMatch) {
    // fallback: grab the flat number
...
function renderEncounterCards() {
  document.getElementById("dmRound").textContent =
    dmState.encounter.round;

  document.getElementById("dmTurn").textContent =
  ...
  dmState.encounter.combatants.forEach((c, i) => {
    ...
    if (i === dmState.encounter.turnIndex) {
...
`;
    ...
    body.querySelectorAll(".condition-chip").forEach(chip => {
      chip.onclick = () => {
        ...
        const i = c.conditions.indexOf(cnd);
        i === -1 ? c.conditions.push(cnd) : c.conditions.splice(i, 1);
        renderEncounterCards();
      };
    });
...
function extractLegendaryActions(monster) {
  ...
  for (const line of monster.contents) {
    ...
    if (inSection && line.startsWith("text |")) {
      // Usually the rules text explaining usage
...
========================= */
document
  .getElementById("addCombatantBtn")
  ?.addEventListener("click", () => {

    console.log("Add Combatant clicked"); // ← TEMP debug
    ...
    if (!name || !maxHp) {
      console.log("Missing name or HP");
      return;
    }
    ...
    console.log("Combatants:", dmState.encounter.combatants);
...
document.getElementById("nextTurnBtn").onclick = () => {
  ...
  if (
    dmState.encounter.turnIndex >=
    dmState.encounter.combatants.length
  ) {
    dmState.encounter.turnIndex = 0;
    dmState.encounter.round++;
  }
...
document.getElementById("saveEncounterBtn").onclick = () => {
  const data = {
...
document.getElementById("loadEncounterBtn").onclick = () => {
  ...
  encounterNameInput.value = dmState.encounter.name;
  dmState.encounter.round = data.round;
  dmState.encounter.turnIndex = data.turnIndex;
  dmState.encounter.combatants = data.combatants.map(c => {
    const monster = c.monsterId
      ? monsters.find(m => m.title === c.monsterId)
      : null;
...
document.getElementById("exportEncounterBtn").onclick = () => {
  const data = {
    version: 1,
    name: dmState.encounter.name ?? "Unnamed Encounter",
    round: dmState.encounter.round,
...
document.getElementById("importEncounterInput").onchange = e => {
  ...
  reader.onload = () => {
    try {
      ...
      dmState.encounter.name = data.name;
      dmState.encounter.round = data.round;
      dmState.encounter.turnIndex = data.turnIndex;
...
document.getElementById("saveToLibraryBtn").onclick = () => {
...
function renderEncounterLibrary() {
  ...
  Object.entries(lib).forEach(([slug, enc]) => {
    ...
    name.onclick = () => loadEncounterFromData(enc);
...
document.getElementById("newEncounterBtn").onclick = () => {
  dmState.encounter = {
    name: "New Encounter",
    round: 1,
    turnIndex: 0,
